...
- 固化国内政府公文版式：`00` 号表必须保持一页并使用可嵌入的宋体；`03/04` 必须从最终 Word 直接导出 PDF，确保页数、行距、签名、公章和分页一致。
- 记录 LibreOffice 字体替换导致行距放大、内容溢页的问题：应使用本机已安装且可嵌入的宋体，并将相对行距换算为视觉等值的固定行距，不能通过缩字、删段或重排模板解决。
- 国内演员名单上传版如有多页，每一页都必须盖章；场地同意函公章必须同时压住场地方公司名称和日期，并完整位于 A4 页面内。
- 固定节目单三列优先宽度为 `12mm / 143mm / 25mm`，序号列仅保留必要空间；加强外文歌词逐句翻译、中文全角括号和标题跨页检查。
- 演员证件有效期必须同时记录开始和截止日期，不得只填写截止日期。
- 涉外材料不添加提供人；涉外消防文件保持原样，不盖章、不加核验文字；歌词删除 Verse、Chorus、Bridge 等非歌词结构标签。
- 新增并验证本地文档工具链：`python-docx 1.2.0`、`python-pptx 1.0.2`、`openpyxl 3.1.5`、`XlsxWriter 3.2.9`、`pandas 3.0.3`、`ReportLab 5.0.0`。
- 公开版继续只包含通用规则、占位配置和去敏文档，不包含真实公司资料、公章、证件、私人字体、固定证明或完成案例。
